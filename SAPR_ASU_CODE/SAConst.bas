...
Public Const typeCxemaActuator As Integer = 100 'Привод вне шкафа. Аналогичен typeCxemaSensor M (Электродвигатель), YA (Клапан электромагнитный)
Public Const typeCxemaSensor As Integer = 110 'Датчик вне шкафа. Содержит несколько Входов/Выходов. (Родительский) RK (Датчик температуры TE) TC (Термопара TE), BP (Датчик давления PT), SP (Реле давления PS), BL (Датчик пламени BE), SQ (Концевик GS), SK (Термостат TS), UZ (Частотник UZ), BN (Сигнализатор загазованности QN)

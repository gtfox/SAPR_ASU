...
'------------------------------------------------------------------------------------------------------------
    ...
    For i = 2 To 8
        ...
        If UBound(mRange) = 0 Then 'выбрана 1 ячейка
            ...
            mVendorData(i + 3) = mRange(1) 'СтолбецАртикул/СтолбецНазвание/СтолбецЦена/СтолбецКатегория/СтолбецГруппа/СтолбецПодгруппа/СтолбецЕдиницы
        Else 'выбран диапазон
            oExcelApp.WindowState = xlMinimized
            MsgBox "Был выбран диапазон ячеек!" & vbCrLf & vbCrLf & "Необходимо выбрать одну ячейку", vbExclamation + vbOKOnly, "САПР-АСУ: Предупреждение"
            i = i - 1
    ...
    wbExcel.Worksheets("НастройкиПрайсов").Activate
    lLastRow = oExcelApp.Sheets("НастройкиПрайсов").Cells(oExcelApp.Sheets("НастройкиПрайсов").Rows.Count, 1).End(xlUp).Row
    For i = 1 To 12
        wbExcel.Sheets("НастройкиПрайсов").Cells(lLastRow + 1, i) = mVendorData(i - 1)
    Next
    oExcelApp.Visible = True
    wbExcel.Save

...
Dim shpChild As Visio.Shape 'шейп из модуля CrossReferenceSensor
...
Sub run(vsoShape As Visio.Shape) 'Приняли шейп из модуля CrossReferenceSensor
    ...
    Select Case FindType
        ...
        Case typeSensor, typeActuator 'Если макрос активировался родителем - значит искали дочерних
            lstvParent.ColumnHeaders.Add , , "Элемент" ' добавить ColumnHeaders
            lstvParent.ColumnHeaders.Add , , "Связь" ' добавить ColumnHeaders
            lstvParent.ColumnHeaders.Add , , "Тип" ' добавить ColumnHeaders
            lstvParent.ColumnHeaders.Add , , "Контур" ' добавить ColumnHeaders
            lstvParent.ColumnHeaders.Add , , "Канал" ' добавить ColumnHeaders
            lstvParent.ColumnHeaders.Add , , "Адрес" ' добавить ColumnHeaders
            lstvParent.ColumnHeaders.Add , , "Страница" ' добавить ColumnHeaders
            lstvChild.Visible = False
            lstvParent.Width = 260
            lblResult.Left = 290
            btnClose.Left = 290
            Me.Width = 376
...
Sub Fill_lstvChild(vsoShape As Visio.Shape) ' заполнение списка дочерних элементов схемы (контактов)
    ...
    'Берем ссылку на дочерний и заполняем список (список из 1 элемента :) )
    If vsoShape.CellsU("Hyperlink.FSA.SubAddress").ResultStr(0) <> "" Then
    ...
    Select Case FindType
        ...
        Case typeSensor, typeActuator 'Если макрос активировался родителем - значит искали дочерних
            For i = 1 To colShapes.Count  ' добавить N ListItem в коллекцию ListItems
                With ActiveDocument.Pages.ItemFromID(colPages.Item(i)).Shapes.ItemFromID(colShapes.Item(i))
                    Set itmx = lstvParent.ListItems.Add(, colPages.Item(i) & "/" & colShapes.Item(i), .Cells("User.Name").ResultStr(0))  '.Cells("TheText").ResultStr("")         .Characters.Text & "/" & .Cells("User.Kontur").ResultStr(0)
                    itmx.SubItems(1) = IIf(.Cells("User.NameParent").ResultStr(0) = "0,0000", "", .Cells("User.NameParent").ResultStr(0))
                    itmx.SubItems(2) = .Cells("Prop.SymName").ResultStr(0)
        ...
        Case typeFSAPodval 'Если макрос активировался typeFSAPodval - значит искали typeFSASensor
            For i = 1 To colShapes.Count  ' добавить N ListItem в коллекцию ListItems
                With ActiveDocument.Pages.ItemFromID(colPages.Item(i)).Shapes.ItemFromID(colShapes.Item(i))
                    Set itmx = lstvParent.ListItems.Add(, colPages.Item(i) & "/" & colShapes.Item(i), .Cells("User.Name").ResultStr(0))  '.Cells("TheText").ResultStr("")         .Characters.Text & "/" & .Cells("User.Kontur").ResultStr(0)
                    itmx.SubItems(1) = IIf(.Cells("Prop.KanalNumber").Result(0) = "0", "", .Cells("Prop.KanalNumber").Result(0)) '
                    itmx.SubItems(2) = .Cells("Prop.SymName").ResultStr(0)
                    itmx.SubItems(3) = .Cells("Prop.NameKontur").ResultStr(0)
...
Private Sub lstvParent_DblClick()
    ...
    Select Case FindType
        ...
        Case typeSensor, typeActuator 'Если макрос активировался родителем - значит искали дочерних
            ...
            AddReferenceSensor shpParent, shpChild
        Case typeFSAPodval
            'Создаем связь как и было задумано
            AddReferenceFSAPodval shpChild, shpParent
    End Select
